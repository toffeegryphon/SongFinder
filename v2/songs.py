...
def getSongsByArtistId(artistId, offset=None, songlist=None):
    ...
    while len(songlist) < recordingCount:
        ...
        print(len(songlist))
        with open("data/%s_raw.json" % (artistId), 'w') as songsFile:
...
def buildArtist(artistName, getAlbumsOfSongs = False):
    ...
    if os.path.isfile('./data/%s.json' % (artistId)):
    ...
    if os.path.isfile('./data/%s_clean.json' % (artistId)):
        print('./data/%s_clean.json' % (artistId))
        with open('./data/%s_clean.json' % (artistId), 'r') as songsFile:
    ...
    with open('data/%s.json' % (artistId), 'w') as artistFile:
...
def addRelationship(secondArtistName, mainArtistName):
    ...
    if os.path.isfile('./data/%s.json' % (mainId)):
        with open('./data/%s.json' % (mainId), 'r') as mainArtistFile:
            mainArtist = json.load(mainArtistFile)
    else:
        mainArtist = buildArtist(mainArtistName)

    if os.path.isfile('./data/%s.json' % (secondId)):
        with open('./data/%s.json' % (secondId), 'r') as secondArtistFile:
    ...
    with open('./data/%s.json' % (mainId), 'w') as mainArtistFile:
        json.dump(mainArtist, mainArtistFile, indent = 4)

    with open('./data/%s.json' % (secondId), 'w') as secondArtistFile:
...
def upvote(artistName, trackName):
    ...
    for track in artist.get('recordings'):
        if track.get('title') == trackName:
            ...
            with open('./data/%s.json' % (artistId), 'w') as file:
...
def sortRecordings(artistRep, artistInput, sortOrder = Sort.ALPHABETICAL):
    ...
    with open('./data/%s.json' % (artistRep), 'r') as file:
